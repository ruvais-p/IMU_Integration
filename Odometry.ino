...
float left_ticks = 0;
float right_ticks = 0;
float last_left_ticks = 0;
float last_right_ticks = 0;
...
Encoder front_right_Encoder(Encoder_output_B2, Encoder_output_A2);
//Encoder middle_right_Encoder(Encoder_output_A1, Encoder_output_B1);
Encoder back_right_Encoder(Encoder_output_B4, Encoder_output_A4);
...
void loop()
{
  ...
  left_ticks = (back_left_Encoder.read() + front_left_Encoder.read())/2;
  right_ticks = (front_right_Encoder.read() + back_right_Encoder.read())/2;
  float delta_L = left_ticks - last_left_ticks;
  float delta_R = right_ticks - last_right_ticks;
  last_left_ticks = left_ticks;
  last_right_ticks = right_ticks;

  float dl = 2 * PI * wheelRadius * delta_L / TPR;
  float dr = 2 * PI * wheelRadius * delta_R / TPR;
  float d = (dl + dr) / 2;
  ...
  if (dl == dr){
    ...
    radius = d / dtheta;
    float iccX = x - radius * sin(theta);
    float iccY = y - radius * cos(theta);

    dx = cos(dtheta) * (x-iccX) - sin(dtheta) * (y-iccY) + iccX - x;
    dy = sin(dtheta) * (x-iccX) + cos(dtheta) * (y-iccY) + iccY - y;

    //dx = d * cos(theta + dtheta / 2.0);
    //dy = d * sin(theta + dtheta / 2.0);
